...
| Channel-Aware AI Replies | Email/WhatsApp/SMS/Call each get tailored prompt | ✅ Complete |
...
| GitHub showing Claude as contributor | `Co-Authored-By: Claude` in commit messages | Recreated repo twice with squash commit — never add Co-Authored-By to future commits |
| AI replies same format across all channels | `generate_reply()` had no channel awareness | Added `CHANNEL_INSTRUCTIONS` per channel + passed `channel` param in `ticket_service.py` |
